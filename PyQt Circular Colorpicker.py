...
class defaultColors(QtWidgets.QWidget):
    def __init__(self, colorpickerWidget, parent=None):
        ...
        self.setStyleSheet(
            "QPushButton:hover{border: 2px solid %s;} QPushButton{border: 0px solid black; max-width: %s; max-height: "
            "%s; min-width: %s; min-height: %s; border-radius: %s;}" % (
...
class ColorPicker(QtWidgets.QWidget):
    def __init__(self, app, width, startupcolor, parent=None):
        ...
        self.sliders = colorpicker_sliders(slidersWidgetWidth=width, spaceBetweenColorpickerAndSliders=0,
        ...
        self.colorpickerWidget = colorpickerWheel(colorpickerSize=width, startupcolor=self.startup_color,
        ...
        self.fftBtn = QtWidgets.QPushButton(text="FFT")
        self.fftBtn.setStyleSheet(
            "border: 3px solid black; border-radius: 5;background: qlineargradient(x1:0, y1:0, x2:0, y2:1,"
            "stop:0 #f4f4f4, stop:0.1 #8F8F8F, stop:1  #7B7B7B);font-weight: bold;color: white;min-width: "
            "70px;height: 25;")

        # self.fftBtn.setStyleSheet("border: 2px solid black; border-radius: 5; color : white; height: 25;")
        ...
        self.layout.addWidget(self.fftBtn)
        self.layout.addStretch(1)

        self.setupTrayIcon()
        self.setupTrayActions()

        self.show()

        self.screenWidth = QtWidgets.QApplication.primaryScreen().availableGeometry().width()
        self.screenHeight = QtWidgets.QApplication.primaryScreen().availableGeometry().height()

        self.move(self.screenWidth - self.width(), 0)

    @QtCore.pyqtSlot()
    def fftBtnClicked(self):
        try:
            requests.post('http://192.168.2.200/FFTPressed')
        except:
            pass

    def setupTrayActions(self):
        self.quitOpt = QtWidgets.QAction("Quit")

        self.quitOpt.triggered.connect(self.app.quit)
        self.menu.addAction(self.quitOpt)

        self.tray.activated.connect(self.regainFocus)
        self.tray.setContextMenu(self.menu)

    def setupTrayIcon(self):
        self.app.setQuitOnLastWindowClosed(False)
        self.tray = QtWidgets.QSystemTrayIcon(QtGui.QIcon("brush.jpg"), self)
    ...
    def onCurrentColorChanged(color):
        ...
        '''colorStr = '{},{},{}'.format(ColorPicker.rgb_color_array[0],
                                     ColorPicker.rgb_color_array[1],
                                     ColorPicker.rgb_color_array[2]
                                     )'''
        try:
            colorData = {'r': ColorPicker.rgb_color_array[0],
                         'g': ColorPicker.rgb_color_array[1],
                         'b': ColorPicker.rgb_color_array[2]
                         }
            requests.post('http://192.168.2.200/colorChanged', data=colorData)
        except:
            pass
...
def run():
    ...
    Colorpicker = ColorPicker(app, width=200, startupcolor=[0, 255, 255])  # HSV (0-360, 0-255, 0-255)
